...
@ban_cmd.handle()
async def ban_someone(event: GroupMessageEvent, matcher: Matcher, args: Message = CommandArg()):
    if get_privilege(get_user_id(event), perm.ADMIN):
        user_id = await check_if_number_user_id(event, args.extract_plain_text().strip())
