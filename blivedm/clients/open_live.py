    ...
    async def _start_game(self):
        try:
            async with self._request_open_live(
                ...
                data = await res.json()
                if data['code'] != 0:
                    logger.warning(f'_start_game() failed, code={data["code"]},'
                                   f' message={data["message"]}, request_id={data["request_id"]}')
                    return False
        ...
        try:
            async with self._request_open_live(
                ...
                if code != 0:
                    ...
                    logger.warning(f'room={self._room_id} _end_game() failed,'
                                   f' code={code}, message={data["message"]}, request_id={data["request_id"]}')
                    return False
        except (aiohttp.ClientConnectionError, asyncio.TimeoutError):
    ...
    async def _send_game_heartbeat(self):
        ...
        try:
            ...
            async with self._request_open_live(
                ...
                if res.status != 200:
                    logger.warning(f'room={self._room_id} _send_game_heartbeat() failed,'
                                   f' status={res.status}, reason={res.reason}')
                    return False
                data = await res.json()
                code = data['code']
                if code != 0:
                    logger.warning(f'room={self._room_id} _send_game_heartbeat() failed, code={code},'
                                   f' message={data["message"]}, request_id={data["request_id"]}')
