...
class WebSocketClientBase:
    ...
    def start(self):
        ...
        """
        if self.is_running:
            logger.warning(f'room={self._room_id} client is running, cannot start() again')
            return
    ...
    def stop(self):
        ...
        if not self.is_running:
            logger.warning(f'room={self._room_id} client is stopped, cannot stop() again')
            return
    ...
    async def _network_coroutine(self):
        ...
        while True:
            ...
            # 准备重连
            retry_count += 1
            logger.warning(f'room={self.room_id} is reconnecting, retry_count={retry_count}')
    ...
    async def _parse_ws_message(self, data: bytes):
        ...
        if header.operation in (Operation.SEND_MSG_REPLY, Operation.AUTH_REPLY):
            ...
            # 未知消息
            body = data[offset + header.raw_header_size: offset + header.pack_len]
            logger.warning(f'room={self.room_id} unknown message'
                           f' operation={header.operation}, header={header}, body={body}')
    ...
    async def _parse_business_message(self, header: HeaderTuple, body: bytes):
        ...
        if header.operation == Operation.SEND_MSG_REPLY:
            ...
            if header.ver == ProtoVer.BROTLI:
                ...
                # 未知格式
                logger.warning(f'room={self.room_id} unknown protocol version={header.ver},'
                               f' header={header}, body={body}')
            ...
            # 未知消息
            logger.warning(f'room={self.room_id} unknown message operation={header.operation}'
                           f' header={header}, body={body}')
    ...
    def _handle_command(self, command: dict):
        ...
        try:
            ...
            logger.exception(f'room={self.room_id} _handle_command() failed, command={command} {e.__class__}')
